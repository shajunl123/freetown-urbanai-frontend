...
const App: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await streamResponse(messages, userMsg.text, (text, groundingChunks) => {
        setMessages(prev => prev.map(msg => 
          msg.id === aiMsgId 
            ? { ...msg, text, isThinking: false, groundingChunks } 
            : msg
        ));
      });
    } catch (error) {
  ...
  return (
    <div className="relative h-screen w-full text-white font-sans overflow-hidden flex flex-col bg-transparent">
      
      {/* 3D Background Layer */}
      <FreetownMap />

      {/* Top Banner / Header */}
      <header className="pointer-events-none h-16 z-20 mx-4 mt-4 flex items-center justify-between shrink-0">
      ...
      {/* Main Grid Layout */}
      <div className="flex-1 p-4 flex gap-4 overflow-hidden relative z-10 pointer-events-none">
        
        {/* LEFT PANEL */}
        <LeftPanel />

        {/* CENTER PANEL */}
        <main className="pointer-events-auto flex-1 rounded-xl border border-white/10 flex flex-col overflow-hidden relative shadow-2xl bg-transparent backdrop-blur-[1px]">
            {/* Chat Header */}
            <div className="p-4 border-b border-white/10 bg-transparent flex justify-between items-center">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-mono text-gray-300 shadow-black text-shadow font-medium">SECURE CHANNEL // CONTEXT: STRUCTURAL PLAN 2028</span>
                </div>
                <div className="flex gap-2">
                    <button className="p-1.5 hover:bg-white/10 rounded text-gray-300 transition-colors">
                ...
                <form onSubmit={handleSubmit} className="flex gap-2 items-end">
                    ...
                    <div className="flex-1 relative group">
                        <input
                            ...
                            className="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-3 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-freetown-neonBlue/50 focus:ring-1 focus:ring-freetown-neonBlue/50 transition-all font-light shadow-inner hover:bg-white/15"
                        />
